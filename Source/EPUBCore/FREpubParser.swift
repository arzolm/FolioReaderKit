...
class FREpubParser: NSObject, SSZipArchiveDelegate {
    ...
    func parseCoverImage(_ epubPath: String, unzipPath: String? = nil) throws -> UIImage {
        ...
        guard let coverImage = (try? readEpub(epubPath: epubPath, removeEpub: false, unzipPath: unzipPath))?.coverImage else {
    ...
    func parseTitle(_ epubPath: String, unzipPath: String? = nil) throws -> String {
        ...
        guard let title = (try? readEpub(epubPath: epubPath, removeEpub: false, unzipPath: unzipPath))?.title else {
    ...
    func parseAuthorName(_ epubPath: String, unzipPath: String? = nil) throws -> String {
        ...
        guard let authorName = (try? readEpub(epubPath: epubPath, removeEpub: false, unzipPath: unzipPath))?.authorName else {
    ...
    func readEpub(epubPath withEpubPath: String, removeEpub: Bool = true, unzipPath: String? = nil) throws -> FRBook {
        ...
        guard fileManager.fileExists(atPath: withEpubPath) else { throw FolioReaderError.bookNotAvailable }
    ...
    private func readContainer(with bookBasePath: String) throws {
        ...
        guard let href = xmlDoc.root["rootfiles"]["rootfile"].attributes["full-path"] else { throw FolioReaderError.fullPathEmpty }
        
        let opfResource = FRResource()
        opfResource.href = href
        opfResource.mediaType = .by(fileName: href)
        book.opfResource = opfResource
        
        resourcesBasePath = bookBasePath.appendingPathComponent(href.deletingLastPathComponent)
    }
    ...
    private func readOpf(with bookBasePath: String) throws {
        ...
        if let identifier = identifier,
            let uniqueIdentifier = book.metadata.find(identifierById: identifier) {
            
            book.uniqueIdentifier = uniqueIdentifier.value
        }

        // Read the cover image
        if let coverImageId = book.metadata.find(byName: "cover")?.content,
           let coverResource = book.resources.findById(coverImageId) {
        ...
        if let tocResource = book.resources.findByMediaType(.ncx) {
    ...
    private func readSmilFileElements(_ children: [AEXMLElement]) -> [FRSmilElement] {
        ...
        children.forEach { element in
            ...
            let smil = FRSmilElement(element: element)

            // if this element has children, convert them to objects too
            
            if element.children.count > 0 {
                
                smil.children.append(contentsOf: readSmilFileElements(element.children))
            }
            data.append(smil)
        }
        return data
    ...
    private func findTableOfContents() -> [FRTocReference] {
        ...
        guard let tocResource = book.tocResource else { return [] }
        ...
        do {
            ...
            if tocResource.mediaType == .ncx {
                ...
                if let nav = findNavTag(xmlDoc.root["body"]), let itemsList = nav["ol"]["li"].all {
        ...
        guard let items = tocItems else { return [] }

        for item in items {
            
            if let ref = readTOCReference(item) {
    ...
    fileprivate func readTOCReference(_ navpointElement: AEXMLElement) -> FRTocReference? {
        ...
        if book.tocResource?.mediaType == .ncx {
            ...
            if let navPoints = navpointElement["navPoint"].all {
                ...
                for navPoint in navPoints {
                    ...
                    if let item = readTOCReference(navPoint) {
    ...
    fileprivate func readMetadata(_ tags: [AEXMLElement]) -> FRMetadata {
        ...
        for tag in tags {
            ...
            if tag.name == "meta" {
                ...
                if let property = tag.attributes["property"] {
                    
                    if let id = tag.attributes["id"] {
                        
                        metadata.metaAttributes.append(Meta(id: id, property: property, value: tag.value))
                        
                    } else {
                    
                        metadata.metaAttributes.append(Meta(property: property, value: tag.value, refines: tag.attributes["refines"]))
                    }
                }
            }
        }
    ...
    fileprivate func readSpine(_ tags: [AEXMLElement]) -> FRSpine {
        ...
        for tag in tags {
            ...
            if let idref = tag.attributes["idref"] {
                ...
                if book.resources.containsById(idref) {
                    ...
                    if let resource = book.resources.findById(idref) {
    ...
    func zipArchiveWillUnzipArchive(atPath path: String, zipInfo: unz_global_info) {
        ...
        guard let epubPathToRemove = epubPathToRemove, shouldRemoveEpub else { return }
